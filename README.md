...
- [Блокировка одновременного обновления одной сущности](#%D0%91%D0%BB%D0%BE%D0%BA%D0%B8%D1%80%D0%BE%D0%B2%D0%BA%D0%B0-%D0%BE%D0%B4%D0%BD%D0%BE%D0%B2%D1%80%D0%B5%D0%BC%D0%B5%D0%BD%D0%BD%D0%BE%D0%B3%D0%BE-%D0%BE%D0%B1%D0%BD%D0%BE%D0%B2%D0%BB%D0%B5%D0%BD%D0%B8%D1%8F-%D0%BE%D0%B4%D0%BD%D0%BE%D0%B9-%D1%81%D1%83%D1%89%D0%BD%D0%BE%D1%81%D1%82%D0%B8)
    - [Использование пользовательского хранилища блокировок](#%D0%98%D1%81%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D0%BD%D0%B8%D0%B5-%D0%BF%D0%BE%D0%BB%D1%8C%D0%B7%D0%BE%D0%B2%D0%B0%D1%82%D0%B5%D0%BB%D1%8C%D1%81%D0%BA%D0%BE%D0%B3%D0%BE-%D1%85%D1%80%D0%B0%D0%BD%D0%B8%D0%BB%D0%B8%D1%89%D0%B0-%D0%B1%D0%BB%D0%BE%D0%BA%D0%B8%D1%80%D0%BE%D0%B2%D0%BE%D0%BA)
...
| `$lockEntityDir`        | 'lock/'                 | Устанавливает каталог для хранения lock-файлов блокировки обновления сущностей при вызове метода `AmoObject::save()`. Используется только если `$lockStore` не задан                                  |
| `$lockStore`            | null                    | Устанавливает хранилище блокировок `symfony/lock` (должен реализовывать `PersistingStoreInterface`). Если null, используется `FlockStore`. Пример: `AmoAPI::$lockStore = new RedisStore($redis);`     |
...
### Использование пользовательского хранилища блокировок

По умолчанию для хранения блокировок используется `FlockStore` — файловые блокировки в каталоге `AmoAPI::$lockEntityDir`.
Этот механизм работает только в рамках одного сервера и файловой системы. В распределённых окружениях,
где несколько серверов обращаются к одному аккаунту amoCRM, файловые блокировки не обеспечивают
межпроцессную синхронизацию между машинами.

Для таких случаев можно подключить любое совместимое хранилище из пакета `symfony/lock`, реализующее
интерфейс `PersistingStoreInterface`: `RedisStore`, `MemcachedStore`, `PdoStore`, `ZookeeperStore` и др.

Хранилище задаётся через свойство `AmoAPI::$lockStore` **до первого вызова** любого метода, использующего блокировки:

```php
use AmoCRM\AmoAPI;
use Symfony\Component\Lock\Store\RedisStore;

// Использование Redis для хранения блокировок (несколько серверов)
$redis = new \Redis();
$redis->connect('127.0.0.1', 6379);
AmoAPI::$lockStore = new RedisStore($redis);
```
...
// Использование Memcached для хранения блокировок
$memcached = new \Memcached();
$memcached->addServer('127.0.0.1', 11211);
AmoAPI::$lockStore = new MemcachedStore($memcached);
```
...
// Использование базы данных для хранения блокировок
AmoAPI::$lockStore = new PdoStore('mysql:host=localhost;dbname=mydb', [
    'db_username' => 'user',
    'db_password' => 'password',
]);
```

Если `AmoAPI::$lockStore` не задан (значение по умолчанию `null`), библиотека использует `FlockStore`
с каталогом из `AmoAPI::$lockEntityDir`.
